...
- Add:
	...
	- constants.
	- equality operators to Nullable/Disposable/2 (for both generic and non-generic versions).
	- new types.
- Update array and list shift operations.
- Bitwise shift operations use the >>> operator when applicable.
- Remove the ThrowHelper.ThrowIfNull method for .Net >= 6.0. Use the ArgumentNullException.ThrowIfNull method instead.
- Bug fixes in:
	- the InvalidEnumArgumentException(string message, string argumentName, Enum invalidValue) constructor -- an exception was thrown for some values.
	- bitwise operations.
	- ReadOnlyArrayArray\<T>: the implementation of IULongIndexableR.this[ulong index] threw a NotImplementedException.
