...
TItems
        ...
        public class EnumerableQueueCollection<TQueue, TItems> : IEnumerableQueue<TItems>, IReadOnlyCollection<TItems>, ICollection where TQueue : QueueCollection<IEnumerableQueue<TItems>, TItems>
        {
            ...
            int ICollection.Count => ((ICollection)InnerQueue.InnerQueue).Count;

            int IReadOnlyCollection<TItems>.Count => ((IReadOnlyCollection<TItems>)InnerQueue.InnerQueue).Count;

            object ISimpleLinkedListBase2.SyncRoot => ((ISimpleLinkedListBase2)InnerQueue.InnerQueue).SyncRoot;

            bool ISimpleLinkedListBase2.IsSynchronized => ((ISimpleLinkedListBase2)InnerQueue.InnerQueue).IsSynchronized;

            bool ISimpleLinkedListBase.IsReadOnly => InnerQueue.IsReadOnly;

            bool ISimpleLinkedListBase.HasItems => InnerQueue.InnerQueue.HasItems;
            ...
            /// </summary>
            /// <param name="queue">The inner queue for this <see cref="QueueCollection{T}"/>.</param>
            public EnumerableQueueCollection(in TQueue queue) => InnerQueue = queue;
            ...
            /// <param name="item">The object to locate in the <see cref="QueueCollection{T}"/>. The value can be <see langword="null"/> for reference types.</param>
            /// <returns><see langword="true"/> if <paramref name="item"/> is found in the queue; otherwise, <see langword="false"/>.</returns>
            public bool Contains(TItems item) => InnerQueue.InnerQueue.Contains(item);

            /// <summary>
            /// Copies the <see cref="QueueCollection{T}"/> elements to a new array.
            /// </summary>
            /// <returns>A new array containing elements copied from the <see cref="QueueCollection{T}"/>.</returns>
            public TItems[] ToArray() => InnerQueue.InnerQueue.ToArray();
            ...
            /// <exception cref="ArgumentOutOfRangeException"><paramref name="arrayIndex"/> is less than zero.</exception>
            /// <exception cref="ArgumentException">The number of elements in the source <see cref="QueueCollection{T}"/> is greater than the available space from <paramref name="arrayIndex"/> to the end of the destination array.</exception>
            public void CopyTo(TItems[] array, int arrayIndex) => InnerQueue.InnerQueue.CopyTo(array, arrayIndex);

            TItems IQueue<TItems>.Peek() => InnerQueue.Peek();

            bool IQueue<TItems>.TryPeek(out TItems result) => InnerQueue.TryPeek(out result);

            void IQueue<TItems>.Clear() => InnerQueue.Clear();

            void IQueueBase<TItems>.Enqueue(TItems item) => InnerQueue.Enqueue(item);

            TItems IQueueBase<TItems>.Dequeue() => InnerQueue.Dequeue();

            bool IQueueBase<TItems>.TryDequeue(out TItems result) => InnerQueue.TryDequeue(out result);

            TItems IQueueBase<TItems>.Peek() => InnerQueue.Peek();

            bool IQueueBase<TItems>.TryPeek(out TItems result) => InnerQueue.TryPeek(out result);

            void IQueueBase<TItems>.Clear() => InnerQueue.Clear();

            TItems ISimpleLinkedListBase<TItems>.Peek() => InnerQueue.Peek();
        ...
        public class EnumerableQueueCollection<T> : EnumerableQueueCollection<QueueCollection<IEnumerableQueue<T>, T>, T>
        {
            ...
            /// </summary>
            /// <param name="queue">The inner queue for this <see cref="QueueCollection{T}"/>.</param>
            public EnumerableQueueCollection(in QueueCollection<IEnumerableQueue<T>, T> queue) : base(queue) { }
        }
    }
#endif
}
