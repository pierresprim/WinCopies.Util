...
Commands
{
    ...
    public static class EventHandlerGenerator
    {
        ...
        /// </summary>
        /// <param name="eventHandlerType">The delegate type to wrap. Note that this must always be a <see langword="void"/> delegate.</param>
        /// <param name="methodToInvoke">The method to invoke.</param>
        /// <param name="methodInvoker">The object where the method resides.</param>
        /// <returns>Returns a delegate with the same signature as <paramref name="eventHandlerType"/> that calls the methodToInvoke inside.</returns>
        public static Delegate CreateDelegate(Type eventHandlerType, MethodInfo methodToInvoke, object methodInvoker)
        {
            ...
            MethodInfo
                ...
                ?
#endif
                eventHandlerInfo = (typeof(Delegate).IsAssignableFrom(eventHandlerType) ? eventHandlerType : throw new ArgumentException($"The {nameof(Delegate)} type must be assignable from {nameof(eventHandlerType)}")).GetMethod(nameof(MethodInfo.Invoke));

            ParameterInfo[] delegateParameters = ((eventHandlerInfo ?? throw new ArgumentException($"The {nameof(Delegate)} type does not implement the {nameof(MethodInfo.Invoke)} method.")).ReturnParameter.ParameterType == typeof(void) ? eventHandlerInfo : throw new ApplicationException("Delegate has a return type. This only supprts event handlers that are void")).GetParameters();
